...
#define GetRowCol(__row, __col, __id) \
    int __row = m_s[__id]._row;\
    int __col = m_s[__id]._col
...
void Board::drawStone(QPainter &p, int id)
{
    ...
    p.setFont(QFont("system", _r*2.0, 75));
...
{
    return center(m_s[id]._row, m_s[id]._col);
}
...
{
    if(killid != -1 && m_s[killid]._type == Stone::JIANG)
...
int Board::getStoneId(int row, int col)
{
    for(int i=0; i<32; ++i)
    {
        if(m_s[i]._row == row && m_s[i]._col == col && !isDead(i))
            return i;
    }
    return -1;
}
